...
Both items come from the same user field report (issue #73), which is now the deepest integration anyone has run against FusionCore: Nav2 GPS waypoint following.
...
- **Documented a defect in the bundled Nav2 configuration that stops the robot moving.** `fusioncore_nav2.launch.py` includes nav2_bringup's `navigation_launch.py`, which starts `collision_monitor` unconditionally (Jazzy has no launch argument to disable it) and lifecycle-manages it, while the bundled `nav2_params.yaml` has no `collision_monitor` section. That node refuses to configure without one (`parameter 'observation_sources' is not initialized`, reproduced against three different configurations), and it sits in the command path, subscribing to `cmd_vel_smoothed` and republishing `cmd_vel`. So planning and control run correctly and produce velocities that never reach the base. `observation_sources` is mandatory and cannot be empty, so a lidar-less GPS robot has no pass-through fallback. Not yet fixed in the shipped config, since a correct fix needs validating against a real Nav2 stack; `docs/nav2.md` now carries the diagnosis and two workarounds. The filter is unaffected either way and keeps publishing `/fusion/odom` and TF normally. Found by the reporter of issue #73, who had to disable the collision monitor to get Nav2 up.
