...
- **A discarded GPS track-heading window now says so in `GnssFixDebug`.** #109 added the branch that throws a window away when a turn happened inside it, and it was the one branch that wrote neither `track_heading_state` nor `track_heading_baseline_m`. Both fields are only assigned from inside branches and `gnss_debug_` is cleared wholesale only in `init()`/`reset()`, so on a discard fix a bag showed the previous fix's verdict verbatim — typically `MOTION_UNSUITABLE` from the corner itself, next to a baseline from before it. New `TrackHeadingState::WINDOW_HAD_TURN`, and the baseline reports the displacement being discarded, since a long discarded window is a different story from a short one. Closes #112.
