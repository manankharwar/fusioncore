...
| `MIN_SATS` on every single fix | `gnss.min_satellites` above 4 on a `sensor_msgs/NavSatFix` input. The message has no satellite count, so it is synthesised as 4 and the threshold can never be met — the filter dead-reckons for the whole run. The node warns about this at startup | Lower `gnss.min_satellites` to 4 or less, or switch to `gps_msgs/GPSFix` with `gnss.use_gps_fix: true`, which carries a real `status.satellites_used` |
